...
const EditProfile = ({ errosMesages, profile, createErrors, createProfile, getCurrentProfile, history }) => {
  ...
  useEffect(() => {
    if (profile) {
      ...
      const skillsField = profile.skills.join(', ');

      formArray.forEach(value => {
        oldProfile[value] = !isEmpty(profile[value]) ? profile[value] : '';
      });

      setForm({ ...oldProfile, skills: skillsField });
    }
...
export default connect(mapStateToProps, { createErrors, createProfile, getCurrentProfile })(EditProfile);
